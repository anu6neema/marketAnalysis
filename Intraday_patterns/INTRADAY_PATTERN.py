    ...
    def IsCandleMediumBody(self, df_):
        ...
        try:            
            ...
            if GR != 'NONE':                
                if BODY > UWICK + LWICK:
                    chk = True
                    candlePattern = 'IsCandleMediumBody' 
##            if GR != 'NONE':                
        ...
        try:            
            ...
            if GR =='GREEN' and CLOSE > OPEN:
                ...
                if UPDOWN == 'UP' and LWICK > 1.5*BODY and LWICK > 2*UWICK:
                    candlePattern = 'IsCandlePinbar 1' 
                    chk = True
                elif UPDOWN == 'DOWN' and UWICK >1.5*BODY and UWICK > 2*LWICK:
                    candlePattern = 'IsCandlePinbar 2' 
                    chk = True
                elif UPDOWN == 'UP' and BODY > 2*LWICK and LWICK > 2*UWICK:
                    candlePattern = 'IsCandlePinbar 3' 
                    chk = True
##                elif UPDOWN == 'UP' and BODY > LWICK and BODY > UWICK and LWICK > 2*UWICK:
##                    chk = True                     
            elif GR == 'RED' and CLOSE < OPEN:
                UWICK = HIGH - OPEN
                LWICK = CLOSE - LOW
                if UPDOWN == 'UP' and LWICK > 1.5*BODY and LWICK > 2*UWICK:
                    candlePattern = 'IsCandlePinbar 4' 
                    chk = True
                elif UPDOWN == 'DOWN' and UWICK > 1.5*BODY and UWICK > 2*LWICK:
                    candlePattern = 'IsCandlePinbar 5' 
                    chk = True
                elif UPDOWN == 'DOWN' and BODY > 2*LWICK and UWICK > 2*LWICK:
                    candlePattern = 'IsCandlePinbar 6' 
                    chk = True
##                elif UPDOWN == 'DOWN' and BODY > LWICK and BODY > UWICK  and UWICK > 2*LWICK:
##                    chk = True                      
        except Exception as e:
            print(e)
            chk = False
...
##import pandas as pd
##df_ = pd.DataFrame()
##df_['OPEN']     = [2253.0, 204.7, 12084.75]
##df_['HIGH']     = [2256.5, 204.7, 12142.25]
##df_['LOW']      = [2242.1, 203.55, 12005.00]
##df_['CLOSE']    = [2246.65, 204.1, 12134.20]
##
##print(df_.iloc[2])
####2253.0,2256.5,2242.1,2246.65
##ptn = patterns()
##print(df_.iloc[2]['OPEN']) 7.5

##bull, pattern = ptn.IsCandleMediumBody(df_.iloc[0])
##print(bull)
##print(pattern)
##print(ptn.IsCandlePinbar(df_.iloc[0],'DOWN','RED'))
##print(ptn.IsCandleMediumBody(df_.iloc[0]))
##print(ptn.IsCandleBigBody(df_.iloc[0]))
